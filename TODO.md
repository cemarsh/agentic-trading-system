...
- [ ] **Item 4 — smarter auto-recovery** — replace exact `api_failures` count with "last successful API call recent → probe + auto-clear"; only stay halted for genuine auth (401/403-auth) errors. *(Lower priority now that Item 3 stops most transient errors at the source.)*
